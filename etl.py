...
def process_song_data(spark, input_data, output_data):
    ...
    Arguments:
        ...
        input_data: path to the s3 bucket containing input jeson files.
    ...
    songs_table = df.select(['song_id', 'title', 'artist_id', 'year', 'duration']).distinct().where(col('song_id').isNotNull())
    ...
    artists_table = df.select(['artist_id', 'artist_name', 'artist_location', 'artist_latitude', 'artist_longitude']).distinct().where(col('artist_id').isNotNull())
...
def process_log_data(spark, input_data, output_data):
    ...
    time_table = df.select('ts', 'start_time', 'datetime', hour("datetime").alias('hour'), dayofmonth("datetime").alias('day'), weekofyear("datetime").alias('week'), year("datetime").alias('year'), month("datetime").alias('month'), dayofweek("datetime").alias('weekday')).dropDuplicates()

    # write time table to parquet files partitioned by year and month
    time_table_path = output_data + 'time'
    time_table.write.partitonBy('year', 'month').parquet(time_table_path)
